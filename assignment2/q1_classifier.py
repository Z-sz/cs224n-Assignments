...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.Variable(tf.zeros(shape=[self.config.n_features, self.config.n_classes], dtype=tf.float32), dtype=tf.float32, name='W')
        b = tf.Variable(tf.zeros(shape=[self.config.n_classes], dtype=tf.float32), dtype=tf.float32, name='b')
        x = tf.matmul(self.input_placeholder, W) + b
        pred = softmax(x)
        ### END YOUR CODE
        return pred
