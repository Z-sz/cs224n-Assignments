...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        sentence_ext = [start for i in range(window_size)]
        sentence_ext.extend(sentence)
        sentence_ext.extend([end for i in range(window_size)])
        for i in range(len(sentence)):
            cur_embed = []
            for j in range(2 * window_size + 1):
                cur_embed.extend(sentence_ext[i + j])
            windowed_data.append((cur_embed, labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(name='input_placeholder', shape=(None, self.config.n_window_features), dtype=tf.int32)
        self.labels_placeholder = tf.placeholder(name='labels_placeholder', shape=(None,), dtype=tf.int32)
        self.dropout_placeholder = tf.placeholder(name='dropout_placeholder', shape=(), dtype=tf.float32)
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        emb = tf.Variable(self.pretrained_embeddings)  #pretrained_embeddings should be trainable!
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name='W', shape=(input_size, self.config.hidden_size), dtype=tf.float32,
                            initializer=tf.contrib.keras.initializers.he_uniform()) #Maybe He is better.
