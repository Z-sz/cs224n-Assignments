...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        ini_state = tf.zeros(shape=(batch_size, 1))
        _, state = tf.nn.dynamic_rnn(cell=cell, inputs=x, time_major=False, initial_state=ini_state)
        preds = tf.nn.sigmoid(x=state, name='preds')
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            grads_clp, self.grad_norm = tf.clip_by_global_norm(t_list=grads, clip_norm=self.config.max_grad_norm)
            train_op = optimizer.apply_gradients(zip(grads_clp, tf.trainable_variables()))
        else:
            self.grad_norm = tf.global_norm(t_list=grads)
