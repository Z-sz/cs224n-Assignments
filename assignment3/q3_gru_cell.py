...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            # computational graph
            z_t = tf.nn.sigmoid(name='z_t',
                                x=tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z)
            r_t = tf.nn.sigmoid(name='r_t',
                                x=tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r)
            o_t = tf.nn.tanh(name='r_o',
                             x=tf.matmul(inputs, U_o) + r_t * tf.matmul(state, W_o) + b_o)
            new_state = z_t * state + (1 - z_t) * o_t      # may be 1 is not compatible here an all-one matrix
