...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        raw_len = min(max_length, len(sentence))
        pad_len = np.max(max_length - len(sentence), 0)
        sentence_pad = sentence[:raw_len] + [zero_vector] * pad_len
        labels_pad = labels[:raw_len] + [zero_label] * pad_len
        mask = [True] * raw_len + [False] * pad_len
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(name='input', shape=(None, self.max_length, self.config.n_features),
                                                dtype=tf.int32)
        self.labels_placeholder = tf.placeholder(name='labels', shape=(None, self.max_length), dtype=tf.int32)
        self.mask_placeholder = tf.placeholder(name='mask', shape=(None, self.max_length), dtype=tf.bool)
        self.dropout_placeholder = tf.placeholder(name='dropout', shape=(), dtype=tf.float32)
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, shape=(-1, self.config.max_length, self.config.embed_size * self.config.n_features))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h_prev = tf.zeros(shape=(tf.shape(x)[0], self.config.hidden_size), name='h_0')
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(x[:, time_step, :], h_prev)
                o_drop = tf.nn.dropout(o_t, keep_prob=dropout_rate)
        ...
        # preds = tf.reshape(preds, shape=(-1, self.config.n_classes, self.max_length))
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds,
